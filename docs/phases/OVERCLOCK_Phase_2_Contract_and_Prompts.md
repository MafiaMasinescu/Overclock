...
Request kinds: INITIALIZE_NEW(seed, contentVersion, fingerprint); LOAD_SLOT(slotId); COMMAND(command); REQUEST_FULL_SNAPSHOT; ACK_PUBLICATION(publicationSequence); SET_PRESENTATION_CONTEXT(selected IDs, inspected entity, heatmapEnabled); SET_HOST_VISIBILITY(visible); CONTINUE_HOST; REQUEST_SAVE(reason); LIST_SLOTS; PREVIEW_IMPORT(owned file bytes); CONFIRM_IMPORT(token, destination, expectedRevision, applySettings); EXPORT_SLOT(slotId, expectedRevision); DELETE_SLOT(slotId, expectedRevision); UPDATE_SETTINGS; REQUEST_REPORT; LIST_REPORTS; DELETE_REPORT; RECOVER; SHUTDOWN. SAVE/LOAD/IMPORT operations return typed terminal replies. The client surfaces a user-confirmation API; Worker confirmation tokens alone are not an authentication boundary against malicious same-origin code.
